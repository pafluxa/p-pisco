...
config = parse_config_file( sys.argv[1], useRelativePath=True )
...
#TT, EE, BB, TE, EB, TB
oTT,oEE,oBB,_,_,_ = healpy.anafast( (oI,oQ,oU), lmax=lmax, pol=True )
pTT,pEE,pBB,_,_,_ = healpy.anafast( (pI,pQ,pU), lmax=lmax, pol=True )

# Adjust to the lmax parameter, because Spice doesn't have that option
# and convert to uK**2

oTT = oTT[0:lmax+1] * (2*numpy.pi)
oEE = oEE[0:lmax+1] * (2*numpy.pi)
oBB = oBB[0:lmax+1] * (2*numpy.pi)

#sTT = sTT[0:lmax+1] 
#sEE = sEE[0:lmax+1] 
#sBB = sBB[0:lmax+1] 

pTT = pTT[0:lmax+1] * (2*numpy.pi)
pEE = pEE[0:lmax+1] * (2*numpy.pi)
pBB = pBB[0:lmax+1] * (2*numpy.pi)
...
ax_TT.set_ylim( ( 1e2,1e4) )                                                                              
ax_TT.set_xlim( (2,lmax) )                                                                                    
ax_TT.set_yscale('symlog', linthreshy=5e1)                                                                  
