...
for beam_nside in [2048]:
#for beam_nside in [256,512,1024,2048]:
    ...
    maxI = numpy.max( I_smooth )

    print( beam_nside, numpy.max( numpy.abs(I_smooth - I_pa)/maxI ), 
                       numpy.max( numpy.abs(Q_smooth - Q_pa)/maxI ),
                       numpy.max( numpy.abs(U_smooth - U_pa)/maxI ) )
    numpy.savez( outputName,
                 nside=map_nside,
                 coords=(ra_s,dec_s),
                 I=I_pa,
                 Q=Q_pa,
...
fig =pylab.figure()
Ip  = healpy.gnomview( I_pa,     sub=(2,3,1), rot=(ra_s, dec_s), return_projected_map=True , title='PISCO rec. I', notext=True )
Qp  = healpy.gnomview( Q_pa,     sub=(2,3,2), rot=(ra_s, dec_s),return_projected_map=True , title='PISCO rec. Q', notext=True  )
Up  = healpy.gnomview( U_pa,     sub=(2,3,3), rot=(ra_s, dec_s),return_projected_map=True , title='PISCO rec. U', notext=True )
