...
pa2   = numpy.concatenate( (pa0,pa45,pa90) )

# build single detector pointing stream (6 sweeps, 3 with offsets added and 3 with offsets subtracted)
ra  = numpy.concatenate( ( ra1, ra2) )
dec = numpy.concatenate( (dec1,dec2) )
pa  = numpy.concatenate( ( pa1, pa2) )

# tile to duplicate this for the second detector in the psb
ra  = numpy.tile(  ra, (2,1) )
dec = numpy.tile( dec, (2,1) )
pa  = numpy.tile(  pa, (2,1) )
