...
# Smooth window by the beam
window = healpy.smoothing( window, fwhm=numpy.radians( beam_fwhm ) )
...
_TTo, _EEo, _BBo, _,_,_ = healpy.anafast( (I_orig,Q_orig,U_orig), pol=True )

'''Compute power specra'''
TT, EE, BB, TE, EB, TB       = spice( (I    ,Q    ,U    ), window=window )
TTo, EEo, BBo, TEo, EBo, TBo = spice( (I_o_s,Q_o_s,U_o_s), window=window )
...
# Get amount of seen pixels to get the sky fraction
f = 1 - numpy.sum( window  )/(1.*healpy.nside2npix( NSIDE ))
...
# Get window function from pisco
wl_PISCO = numpy.load( './data/stacked_beams.npz' )['window_function']
...
ax_TT.plot( ell2*TTo/wl_TT/f, label='healpy.smoothing $C_\ell^{\mathrm{TT}}$' ) 
ax_TT.plot( ell2*TT/wl_TT/f , label='PISCO $C_\ell^{\mathrm{TT}}$' ) 
ax_TT.legend()

ax_EE.plot( ell2*_EEo, label='input $C_\ell^{\mathrm{EE}}$' ) 
ax_EE.plot( ell2*EEo/wl_EE/f, label='healpy.smoothing $C_\ell^{\mathrm{EE}}$' ) 
ax_EE.plot( ell2*EE/wl_EE/f , label='PISCO $C_\ell^{\mathrm{EE}}$' ) 
ax_EE.legend()

ax_BB.plot( ell2*_BBo, label='input $C_\ell^{\mathrm{BB}}$') 
ax_BB.plot( ell2*BBo/wl_BB/f, label='healpy.smoothing $C_\ell^{\mathrm{BB}}$') 
ax_BB.plot( ell2*BB/wl_BB/f , label='PISCO $C_\ell^{\mathrm{BB}}$' ) 
ax_BB.hlines( 1e-16, 2,250, colors='red' )
...
numpy.savez( "maps.npz", I=I, Q=Q, U=U, weigths=W, window=window )
