...
# Create a window
window = numpy.ones_like( W , dtype='float' )
...
low_cut = -65
hgh_cut =  25
window[ (numpy.pi/2.0 - tht) < low_cut ] = 0
window[ (numpy.pi/2.0 - tht) > hgh_cut ] = 0
...
mask = numpy.copy( window )
window = healpy.smoothing( window.astype('float'), fwhm=4*numpy.radians( beam_fwhm ) )
...
TT, EE, BB, TE, EB, TB       = spice( (I    ,Q    ,U    ), window=window, mask=mask )
TTo, EEo, BBo, TEo, EBo, TBo = spice( (I_o_s,Q_o_s,U_o_s), window=window, mask=mask )
...
ax_TT.plot( ell2*TTo/wl_TT, label='healpy.smoothing $C_\ell^{\mathrm{TT}}$' ) 
ax_TT.plot( ell2*TT/wl_TT , label='PISCO $C_\ell^{\mathrm{TT}}$' ) 
ax_TT.legend()

ax_EE.plot( ell2*_EEo, label='input $C_\ell^{\mathrm{EE}}$' ) 
ax_EE.plot( ell2*EEo/wl_EE, label='healpy.smoothing $C_\ell^{\mathrm{EE}}$' ) 
ax_EE.plot( ell2*EE/wl_EE , label='PISCO $C_\ell^{\mathrm{EE}}$' ) 
ax_EE.legend()

ax_BB.plot( ell2*_BBo, label='input $C_\ell^{\mathrm{BB}}$') 
ax_BB.plot( ell2*BBo/wl_BB, label='healpy.smoothing $C_\ell^{\mathrm{BB}}$') 
