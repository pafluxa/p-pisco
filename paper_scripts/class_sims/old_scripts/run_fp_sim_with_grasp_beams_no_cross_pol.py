...
for uid in uids:
    ...
    print uid, fwhm_x[uid], fwhm_y[uid], rotation[uid]
    ...
    beam_co_a = make_gaussian_elliptical_beam( beam_nside, fwhm_x[uid], fwhm_y[uid], theta=rotation[uid] )
    beam_cx   = 0.0*numpy.abs( E_cx ) / numpy.abs( E_co ).max()
    
    if uid == 39:
        healpy.gnomview( (beam_co)**2, rot=(0,90), xsize=500, sub=(1,2,1), title='GRASP' )
        healpy.gnomview( (beam_co_a)**2, rot=(0,90), xsize=500, sub=(1,2,2), title='Analytical gaussian' )
        pylab.show()
