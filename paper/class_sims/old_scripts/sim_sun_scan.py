...
for det in receiver.uid:
    ...
    data = deproject_sky_for_feedhorn( 
                  ...
                  gpu_dev=0, maxmem=7000, grid_size=0.5 )
    
    detector_data[ det ] = data

# Save TOD to disk
numpy.savez( "sun_scan_tod.npz", 
              data=detector_data,
              invalid_dets=inv_dets,
              ctime=ctime,
              az=az,
              alt=alt,
              rot=rot )
...
numpy.savez( 'matrices_%s.npz' % (args.tag), AtA=AtA, AtD=AtD, nside=map_nside, det_mask=det_mask )
