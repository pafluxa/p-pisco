...
for uid in uids:
    ...
    beam_co   = azel_grid_to_healpix_beam( numpy.abs(E_co), nx, ny, grid_size/2.0, beam_nside ).real
    beam_cx   = azel_grid_to_healpix_beam( numpy.abs(E_cx), nx, ny, grid_size/2.0, beam_nside ).real
    
    '''
    healpy.orthview( 20*numpy.log10(beam_co+0.00001), 
        rot=(0,90), 
        title='GRASP AZEL to HPX', 
        sub=(1,2,1), 
        min=-40, max=40, 
        half_sky=True,
        unit='dBi' )
    healpy.orthview( 20*numpy.log10(beam_cx+0.00001), 
        rot=(0,90), 
        title='GRASP AZEL to HPX', 
        sub=(1,2,2), 
        min=-40, max=0, 
    ...
    pylab.show() 
    '''
    # Set cross polarization pattern to zero
...
for det in receiver.uid:
    ...
    data = deproject_sky_for_feedhorn( 
                  ...
                  gpu_dev=0, maxmem=4096, grid_size=0.6 )
