...
beam_nside  = 657
beam_fwhm   = 1.5
beam_extn   = numpy.radians(7.0) # Extension of 7 degrees
...
# Beam for feedhorn 1
beam0_co  = make_tophat_beam( beam_fwhm, beam_extn, beam_nside )
beam0_cx  = numpy.zeros_like( beam0_co )

# Beam for feedhorn 2
beam90_co  = make_tophat_beam( beam_fwhm, beam_extn, beam_nside )
...
for i,scan_angle in enumerate(scan_angles):
    ...
    tod = deproject_sky_for_feedhorn(
        ...
        beam_extn, beam_nside, beam0_co,beam0_cx, beam90_co, beam90_cx, gpu_dev=2 )
    TOD_pa.append( tod )
