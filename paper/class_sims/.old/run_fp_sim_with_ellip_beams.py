...
for feed in range( receiver.ndets + 1 ):
    ...
    V_co = make_gaussian_elliptical_beam( beam_nside, 1.5, 1.5, theta=0.0 )
    V_cx = numpy.zeros_like( V_co )
    H_co = make_gaussian_elliptical_beam( beam_nside, 1.5, 1.5, theta=0.0 )
    H_cx = numpy.zeros_like( H_co )

    beams[ feed ] = { 'V_co': V_co , 'V_cx': V_cx, 'H_co':H_co, 'H_cx':H_cx, 'nside': 512 }
#----------------------------------------------------------------------------------------------------------#
