...
#include <healpix_math.cuh>

// Porten from Healpix_base.cc
__device__ int
...
__device__ void 
pix2ang_ring_z_phi (int nside_, int pix, double *z, double *phi)                                  
  {                                                                                                           
...
ang2pix_ring_z_phi ( int nside_, double z, double phi, double sth )
  ...
  if (za<=twothird) /* Equatorial region */
    ...
    int jp = __double2int_rd(temp1-temp2); /* index of  ascending edge line */
    int jm = __double2int_rd(temp1+temp2); /* index of descending edge line */
    ...
    {
    double tp = tt - __double2int_rd(tt);
    
    double tmp = -1;
    if( za < 0.990 )
      tmp = nside_*sqrt(3.0*(1.0-za));
    else
      tmp = nside_*sth/sqrt((1.+za)/3.);

    int jp = __double2int_rd(tp*tmp); /* increasing edge line index */
    int jm = __double2int_rd((1.0-tp)*tmp); /* decreasing edge line index */
