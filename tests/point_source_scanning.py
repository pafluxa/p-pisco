...
# Setup map matrices
map_nside = beam_nside
AtA = None
AtD = None

# Setup first raster Scan
ra   = numpy.linspace( -0.05, 0.05, nsamples_per_scan )
dec  = numpy.linspace( -0.05, 0.05, nsamples_per_scan )
...
# Show I map
healpy.gnomview( I/I.max() , xsize=100, ysize=100, reso=10, min= 0.0, max=1.0 , sub=(1,2,1) )  
# Show beam
beam_map = numpy.zeros( healpy.nside2npix( beam_nside ) )
beam_map[ 0:beam1_co.size ] = beam1_co

healpy.gnomview( beam_map*numpy.conj(beam_map) , xsize=100, ysize=100, reso=10, min=0.0, max=1 , sub=(1,2,2), rot=(0,90) )
pylab.show()
