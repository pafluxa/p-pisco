...
map_nside = 128
I_map     = numpy.arange( healpy.nside2npix( map_nside ) )*0
Q_map     = numpy.arange( healpy.nside2npix( map_nside ) )*0
U_map     = numpy.arange( healpy.nside2npix( map_nside ) )*0
V_map     = numpy.arange( healpy.nside2npix( map_nside ) )*0

source_pixels = healpy.ang2pix( map_nside, pi/2.0, 0.0 )
I_map[ source_pixels ] = 1
Q_map[ source_pixels ] = 1

# Setup a simple Gaussian beam of 10 arcmin
beam_nside  = 61
beam_fwhm   = 2.0
beam_extn   = numpy.radians(8.0) # Extension of 5 degrees
...
for scan, scan_angle in enumerate( [0.0,pi/2.0,pi/4.0,-pi/4.0] ):
...
healpy.gnomview( I, sub=(3,2,1) )
healpy.gnomview( I_map , sub=(3,2,2) )

healpy.gnomview( Q, sub=(3,2,3) )
healpy.gnomview( Q_map , sub=(3,2,4) )

healpy.gnomview( U , sub=(3,2,5) )
healpy.gnomview( U_map , sub=(3,2,6) )
pylab.show()

'''
c = numpy.dot( I**2 + Q**2 + U**2, I_map**2 + Q_map**2 + U_map**2 )/numpy.dot( I**2 + Q**2 + U**2, I**2 + Q**2 + U**2 )
c = numpy.sqrt(c)
